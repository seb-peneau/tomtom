...
void Domain::loop() {
    ...
    if (network->isConnected() == false) {
        ...
        if (network->isApCreated() == false && storage->read(SSIDSTORAGE) != "" && connectionInProgress == false) {  // Network is lost try to reconnect
            String ssid = storage->read(SSIDSTORAGE);
            String password = storage->read(PASSSTORAGE);
            if (ssid != "" && password != "") {
                displayWifiAnimation = true;
        ...
        if (displayWifiAnimation == true) {
            ...
            display->displayString(network->getIpAddress());
            displayWifiAnimation = false;
        }
        ota->loop();
